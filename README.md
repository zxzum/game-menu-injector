...
The overlay renders a square-style HUD label in the top-right corner of the screen and prints verbose debug lines to `logcat`. Send `SIGUSR1` to the injector process to toggle the menu state while it is running:
...
### Touch gestures
- Tap roughly in the center of the screen to open the `menu_loader` test panel.
- While it is open, drag the label to move it around; release to drop it in place.
- Tapping the `menu_loader` label again will close the panel and reset it to the top-right anchor.
